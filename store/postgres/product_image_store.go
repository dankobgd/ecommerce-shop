...
func (s PgProductImageStore) Update(id int64, pi *model.ProductImage) (*model.ProductImage, *model.AppErr) {
	...
	if _, err := s.db.NamedExec(q, pi); err != nil {
		return nil, model.NewAppErr("PgProductImageStore.Update", model.ErrInternal, locale.GetUserLocalizer("en"), msgGetProduct, http.StatusInternalServerError, nil)
	}
	return pi, nil
}
...
func (s PgProductImageStore) Get(id int64) (*model.ProductImage, *model.AppErr) {
	q := `SELECT img.id AS img_id, img.url AS img_url, img.created_at AS img_created_at, img.updated_at AS img_updated_at FROM public.product_image img WHERE img.id = $1`
	var img model.ProductImage
	if err := s.db.Get(&img, q, id); err != nil {
		return nil, model.NewAppErr("PgProductImageStore.Get", model.ErrInternal, locale.GetUserLocalizer("en"), msgGetProduct, http.StatusInternalServerError, nil)
	}
	return &img, nil
}
...
func (s PgProductImageStore) GetAll(pid int64) ([]*model.ProductImage, *model.AppErr) {
	q := `SELECT img.id AS img_id, img.url AS img_url, img.created_at AS img_created_at, img.updated_at AS img_updated_at FROM public.product_image img WHERE img.product_id = $1`
	var imgs []*model.ProductImage
	if err := s.db.Select(&imgs, q, pid); err != nil {
		return nil, model.NewAppErr("PgProductImageStore.GetAll", model.ErrInternal, locale.GetUserLocalizer("en"), msgGetProduct, http.StatusInternalServerError, nil)
	}
	return imgs, nil
}
